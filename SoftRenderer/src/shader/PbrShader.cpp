...
void PbrShader::Vertex(glm::vec4& gl_Position, const VertexAttribute& vertex, uint32_t vertexIndex)
{
...
bool PbrShader::Fragment(glm::vec4& gl_fragColor)
{
	...
	glm::vec3 lo { 0.0f };
	glm::vec3 F0 { 0.04f };
	glm::vec3 aoRoughMetal = Sampler2D(uv, aoTexture);
	float ao = aoRoughMetal.r;
	float roughness = aoRoughMetal.g;
	float metalness = aoRoughMetal.b;
...
float PbrShader::DistributionGGX(const glm::vec3& N, const glm::vec3& H, float roughness) 
{
...
float PbrShader::GeometrySmith(const glm::vec3& N, const glm::vec3& V, const glm::vec3& L, float roughness)
{
...
glm::vec3 PbrShader::FreshelSchlick(float cosTheta, const glm::vec3& F0)
{
